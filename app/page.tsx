...
<header><div><p className="eyebrow">LEARNING RULES</p><h1>Review what Pocketview remembers.</h1><p className="sub">Rules use a specific transaction identity, then apply a merchant, category and detail.</p></div><div className="actions"><button className="import" onClick={() => saveRules([...rules, { match: `new rule ${Date.now()}`, merchant: "New merchant", category: "Unclassified", detail: "" }])}>＋ Create rule</button></div></header>
<section className="adminPanel"><div className="adminRow adminHead"><span>MATCH IDENTITY</span><span>MERCHANT</span><span>CATEGORY</span><span>CATEGORY DETAIL</span><span></span></div>{rules.map((r, i) => <div className="adminRow" key={`${r.match}-${i}`}><input value={r.match} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, match: e.target.value } : x))}/><input value={r.merchant || ""} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, merchant: e.target.value } : x))}/><select value={r.category} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, category: e.target.value } : x))}>{allCategories.map(x => <option key={x}>{x}</option>)}</select><input value={r.detail} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, detail: e.target.value } : x))}/><button className="deleteButton" onClick={() => saveRules(rules.filter((_, j) => j !== i))}>Delete</button></div>)}{!rules.length && <div className="emptyState">No rules yet. Create one here or use “Remember & apply” in the workbench.</div>}</section>
</> : view === "master" ? <>
<header><div><p className="eyebrow">MASTER DATA</p><h1>Your permanent taxonomy.</h1><p className="sub">Clean and maintain the approved direction, merchant, category and detail combinations.</p></div><div className="actions"><button className="backButton" onClick={() => { const seen = new Set<string>(), clean = master.filter(x => { const key = [x.direction, x.merchant, x.category, x.detail].map(normalizeKey).join("|"); if (seen.has(key)) return false; seen.add(key); return true; }); saveMaster(clean); const uniqueCats = [...new Map(customCategories.map(x => [normalizeKey(x), x])).values()]; setCustomCategories(uniqueCats); localStorage.setItem("pocketview-categories", JSON.stringify(uniqueCats)); setNotice(`Removed ${master.length - clean.length} duplicate master rows.`); }}>Clean duplicates</button><button className="import" onClick={() => saveMaster([...master, { id: `master-${Date.now()}`, direction: "Outbound", merchant: "", category: "Unclassified", detail: "" }])}>＋ Add master row</button></div></header>
...
<select className="workbenchDate" value={workbenchDirection} onChange={e => setWorkbenchDirection(e.target.value)} aria-label="Transaction direction"><option>All</option><option>Inbound</option><option>Outbound</option></select>
