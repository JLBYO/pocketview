...
const periodDates = filteredTxs.map(t => dateMillis(t.date)), periodDays = periodDates.length ? Math.max(1, Math.floor((Math.max(...periodDates) - Math.min(...periodDates)) / 86400000) + 1) : 0, dailyFlow = periodDays ? displayTotal / periodDays : 0, monthlyFlowAverage = periodDays ? displayTotal / periodDays * 30.44 : 0, largestFlow = [...tileTxs].sort((a, b) => Math.abs(b.amount) - Math.abs(a.amount))[0], tileShare = (value: number) => tileTotal ? Math.round(value / tileTotal * 100) : 0, filteredDates = [...new Set(filteredTxs.map(t => t.date))].sort((a, b) => dateKey(a) - dateKey(b));
...
const openCategory = (name: string) => { setSelectedCategory(name); setDirectionSlice(tileDirection); setCategorySlice(name); setDetailSlice("All"); setMerchantSlice("All"); setAppliedFilters({ ...appliedFilters, direction: tileDirection, category: name, detail: "All", merchant: "All" }); setChartSelection(null); setOverviewTransactionPage(0); setView("category"); };
...
{categoryTiles.map(x =>
<article className="budgetCard clickable" tabIndex={0} role="button" key={x.name} onClick={() => openCategory(x.name)} onKeyDown={e => { if (e.key === "Enter") openCategory(x.name); }}>
