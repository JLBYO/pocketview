...
<section className="slicerPanel"><label>FROM<select value={fromDate} onChange={e => setFromDate(e.target.value)}><option value="">First date</option>{dateOptions.map(x => <option key={x}>{x}</option>)}</select></label><label>TO<select value={toDate} onChange={e => setToDate(e.target.value)}><option value="">Last date</option>{dateOptions.map(x => <option key={x}>{x}</option>)}</select></label><label>CATEGORY DETAIL<select value={detailSlice} onChange={e => { setDetailSlice(e.target.value); setMerchantSlice("All"); }}><option>All</option>{detailOptions.map(x => <option key={x}>{x}</option>)}</select></label><label>MERCHANT<select value={merchantSlice} onChange={e => setMerchantSlice(e.target.value)}><option>All</option>{merchantOptions.map(x => <option key={x}>{x}</option>)}</select></label></section>
...
<button className="remember" disabled={!drafts[t.id]} onClick={() => { const edited = { ...t, ...drafts[t.id] }, rule = { match: t.merchant.toLowerCase(), merchant: edited.merchant, category: edited.category, detail: edited.detail }, nextRules = [...rules.filter(r => r.match !== rule.match), rule], nextTxs = txs.map(x => ruleMatches(x, rule) ? { ...x, merchant: rule.merchant, category: rule.category, detail: rule.detail } : x); setRules(nextRules); localStorage.setItem("pocketview-rules", JSON.stringify(nextRules)); save(nextTxs); setDrafts(d => { const next = { ...d }; delete next[t.id]; return next; }); setNotice(`Rule saved and applied to matching ${edited.merchant} transactions.`); }}>Remember & apply</button>
</div>
</article>)}</section>
</>}<footer>
