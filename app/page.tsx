...
const slicerDateTxs = txs.filter(t => (!fromDate || dateKey(t.date) >= dateKey(fromDate)) && (!toDate || dateKey(t.date) <= dateKey(toDate))), slicerDirectionTxs = slicerDateTxs.filter(t => directionSlice === "All" || directionOf(t) === directionSlice), overviewCategoryOptions = [...new Set(slicerDirectionTxs.map(t => t.category || "Unclassified"))].sort(), slicerCategoryTxs = slicerDirectionTxs.filter(t => categorySlice === "All" || t.category === categorySlice), overviewDetailOptions = [...new Set(slicerCategoryTxs.map(t => t.detail).filter(Boolean))].sort(), slicerDetailTxs = slicerCategoryTxs.filter(t => detailSlice === "All" || t.detail === detailSlice), overviewMerchantOptions = [...new Set(slicerDetailTxs.map(t => t.merchant).filter(Boolean))].sort();
...
const applyOverviewFilters = () => { setAppliedFilters({ from: fromDate, to: toDate, direction: directionSlice, category: categorySlice, detail: detailSlice, merchant: merchantSlice }); setChartSelection(null); setOverviewTransactionPage(0); setDeepDivePage(0); };
const clearOverviewFilters = () => { setFromDate(""); setToDate(""); setDirectionSlice("All"); setCategorySlice("All"); setDetailSlice("All"); setMerchantSlice("All"); setAppliedFilters({ from: "", to: "", direction: "All", category: "All", detail: "All", merchant: "All" }); setChartSelection(null); setOverviewTransactionPage(0); setDeepDivePage(0); };
...
const upload = async (f?: File) => { if (!f)
    ...
    setFromDate(importedFrom); setToDate(importedTo); setDirectionSlice("All"); setCategorySlice("All"); setDetailSlice("All"); setMerchantSlice("All"); setAppliedFilters({ from: importedFrom, to: importedTo, direction: "All", category: "All", detail: "All", merchant: "All" }); setChartSelection(null); setOverviewTransactionPage(0); setDeepDivePage(0);
