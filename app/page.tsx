...
const ruleMatches = (t: Tx, r: Rule) => r.mode === "contains" ? normalizeKey(t.description).includes(normalizeKey(r.match)) : canonicalKey(t) === normalizeKey(r.match);
...
const selectedRuleSource = ruleIdentityOptions.find(([key]) => key === (newRuleMatch || ruleIdentityOptions[0]?.[0]))?.[1], newRuleKeywords = keywordOptions(selectedRuleSource);
...
const clearAll = () => { if (!window.confirm("Clear all transactions, rules, custom categories and master data? This cannot be undone unless you exported a knowledge file.")) return; ["pocketview-v2", "pocketview-rules", "pocketview-categories", "pocketview-master", "pocketview-savings-target"].forEach(k => localStorage.removeItem(k)); setTxs([]); setRules([]); setMaster([]); setCustomCategories([]); setDrafts({}); setNewRuleMatch(""); setNewRuleKeyword(""); setNewRuleMerchant(""); setSavingsTarget(20); clearOverviewFilters(); setQuery(""); setWorkbenchDate("All"); setWorkbenchDirection("All"); setReviewOnly(false); setAppliedQuery(""); setAppliedWorkbenchDate("All"); setAppliedWorkbenchDirection("All"); setAppliedReviewOnly(false); setWorkbenchPage(0); setSelectedCategory("Food"); setNotice("All Pocketview data was cleared. Import a CSV to start learning again."); setView("overview"); };
...
<section className="ruleBuilder keywordBuilder"><div><b>Create a merchant rule</b><small>Choose a transaction, then the word that identifies its clean merchant.</small></div><label>SOURCE TRANSACTION<select aria-label="Transaction for new rule" value={newRuleMatch || ruleIdentityOptions[0]?.[0] || ""} onChange={e => { const source = ruleIdentityOptions.find(([key]) => key === e.target.value)?.[1]; setNewRuleMatch(e.target.value); setNewRuleKeyword(keywordOptions(source)[0] || ""); setNewRuleMerchant(source?.merchant || ""); }} disabled={!ruleIdentityOptions.length}>{ruleIdentityOptions.map(([key, t]) => <option key={key} value={key}>{t.description}</option>)}</select></label><label>DESCRIPTION CONTAINS<select aria-label="Keyword for new rule" value={newRuleKeyword || newRuleKeywords[0] || ""} onChange={e => setNewRuleKeyword(e.target.value)} disabled={!newRuleKeywords.length}>{newRuleKeywords.map(word => <option key={word}>{word}</option>)}</select></label><label className="cleanMerchantField">CLEAN MERCHANT<input aria-label="Clean merchant for new rule" value={newRuleMerchant || selectedRuleSource?.merchant || ""} onChange={e => setNewRuleMerchant(e.target.value)} placeholder="Main merchant name"/></label><button className="import" disabled={!selectedRuleSource || !(newRuleKeyword || newRuleKeywords[0]) || !(newRuleMerchant || selectedRuleSource?.merchant)} onClick={() => { const keyword = newRuleKeyword || newRuleKeywords[0], cleanMerchant = (newRuleMerchant || selectedRuleSource?.merchant || "").trim(); if (!selectedRuleSource || !keyword || !cleanMerchant) { setNotice("Choose a transaction, keyword and clean merchant first."); return; } if (rules.some(r => r.mode === "contains" && normalizeKey(r.match) === normalizeKey(keyword))) { setNotice("A contains rule already exists for that keyword."); return; } saveRules([...rules, { match: keyword, mode: "contains", merchant: cleanMerchant, category: selectedRuleSource.category, detail: selectedRuleSource.detail }]); setNotice(`Created a rule: descriptions containing “${keyword}” will use merchant “${cleanMerchant}”.`); }}>＋ Create merchant rule</button></section>
<section className="ruleCards">{rules.map((r, i) => <article className="ruleCard" key={`${r.match}-${i}`}><div className="ruleCardHead"><div><span>RULE {i + 1} · {r.mode === "contains" ? "KEYWORD MATCH" : "EXACT IDENTITY"}</span><b>{r.mode === "contains" ? `Description contains “${r.match}”` : r.match}</b><small>{txs.filter(t => ruleMatches(t, r)).length} matching transactions</small></div><button className="deleteButton" onClick={() => { const nextRules = rules.filter((_, j) => j !== i); saveRules(nextRules); rebuildTransactions(nextRules); setNotice("Rule deleted and transactions refreshed."); }}>Remove rule</button></div><div className="ruleFields merchantFirst"><label className="cleanMerchantField">CLEAN MERCHANT<input value={r.merchant || ""} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, merchant: e.target.value } : x))}/></label><label>MATCH METHOD<select value={r.mode || "identity"} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, mode: e.target.value as "contains" | "identity" } : x))}><option value="contains">Description contains</option><option value="identity">Exact transaction identity</option></select></label><label>{r.mode === "contains" ? "WORD OR PHRASE" : "TRANSACTION IDENTITY"}<input value={r.match} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, match: e.target.value } : x))}/></label><label>CATEGORY<select value={r.category} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, category: e.target.value, detail: "" } : x))}>{allCategories.map(x => <option key={x}>{x}</option>)}</select></label><label>CATEGORY DETAIL<select value={r.detail} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, detail: e.target.value } : x))}><option value="">Select detail</option>{detailOptions.filter(x => x !== r.detail).map(x => <option key={x}>{x}</option>)}{r.detail && <option>{r.detail}</option>}</select></label></div></article>)}{!rules.length && <div className="emptyState ruleEmpty">No rules yet. Select a transaction and keyword above to create the first merchant rule.</div>}</section>
