...
const saveCombination = (t: Tx) => { const identity = canonicalKey(t), masterItem: MasterItem = { id: identity, direction: directionOf(t), originalMerchant: t.description, merchant: t.merchant, category: t.category, detail: t.detail }, rule: Rule = { match: identity, merchant: t.merchant, category: t.category, detail: t.detail }; saveMaster([...master.filter(x => x.id !== identity), masterItem]); saveRules([...rules.filter(x => normalizeKey(x.match) !== normalizeKey(identity)), rule]); setNotice(`Saved ${t.merchant} to master data and learning rules.`); };
...
<section className="transactions categoryTransactions"><div className="drillTable"><div className="drillRow drillHead"><span>DIRECTION</span><span>MERCHANT</span><span>CATEGORY</span><span>CATEGORY DETAIL</span><span>DATE</span><span>AMOUNT</span><span></span></div>{filteredTxs.map(t => <div className="drillRow" key={t.id}><span className={`directionTag ${directionOf(t).toLowerCase()}`}>{directionOf(t)}</span><input value={t.merchant} onChange={e => update(t.id, { merchant: e.target.value })}/><select value={t.category} onChange={e => update(t.id, { category: e.target.value, detail: "" })}>{allCategories.map(x => <option key={x}>{x}</option>)}</select><select value={t.detail} onChange={e => update(t.id, { detail: e.target.value })}><option value="">Select detail</option>{detailChoices(t).filter(x => x !== t.detail).map(x => <option key={x}>{x}</option>)}{t.detail && <option>{t.detail}</option>}</select><span>{t.date}</span><strong className={t.amount > 0 ? "credit" : ""}>{t.amount > 0 ? "+" : ""}{money(t.amount)}</strong><button className="saveCombo" disabled={!t.merchant || t.category === "Unclassified" || !t.detail} onClick={() => saveCombination(t)}>Add to master & rules</button></div>)}</div></section>
</> : view === "rules" ? <>
...
<section className="adminPanel rulesPanel"><div className="adminRow adminHead"><span>MATCH IDENTITY</span><span>MERCHANT</span><span>CATEGORY</span><span>CATEGORY DETAIL</span><span></span></div>{rules.map((r, i) => <div className="adminRow" key={`${r.match}-${i}`}><select title={r.match} value={r.match} onChange={e => { const selected = ruleIdentityOptions.find(([key]) => key === e.target.value)?.[1]; saveRules(rules.map((x, j) => j === i ? { ...x, match: e.target.value, ...(selected ? { merchant: selected.merchant, category: selected.category, detail: selected.detail } : {}) } : x)); }}><option value={r.match}>{r.match}</option>{ruleIdentityOptions.filter(([key]) => key !== r.match).map(([key, t]) => <option key={key} value={key}>{t.description}</option>)}</select><input value={r.merchant || ""} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, merchant: e.target.value } : x))}/><select value={r.category} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, category: e.target.value, detail: "" } : x))}>{allCategories.map(x => <option key={x}>{x}</option>)}</select><select value={r.detail} onChange={e => saveRules(rules.map((x, j) => j === i ? { ...x, detail: e.target.value } : x))}><option value="">Select detail</option>{detailOptions.filter(x => x !== r.detail).map(x => <option key={x}>{x}</option>)}{r.detail && <option>{r.detail}</option>}</select><button className="deleteButton" onClick={() => { const nextRules = rules.filter((_, j) => j !== i); saveRules(nextRules); rebuildTransactions(nextRules); setNotice("Rule deleted and transactions refreshed."); }}>Delete</button></div>)}{!rules.length && <div className="emptyState">No rules yet. Create one here or use “Remember & apply” in the workbench.</div>}</section>
