...
const totals = useMemo(() => { const income = filteredTxs.filter(x => x.amount > 0).reduce((a, x) => a + x.amount, 0), spent = Math.abs(filteredTxs.filter(x => x.amount < 0).reduce((a, x) => a + x.amount, 0)); return { income, spent, left: income - spent }; }, [filteredTxs]); const spend = [...filteredTxs.filter(t => t.amount < 0).reduce((map, t) => { const name = t.category?.trim() || "Unclassified", current = map.get(name) || { name, spent: 0, count: 0 }; current.spent += Math.abs(t.amount); current.count += 1; map.set(name, current); return map; }, new Map<string, { name: string; spent: number; count: number }>()).values()].sort((a, b) => a.name === "Unclassified" ? 1 : b.name === "Unclassified" ? -1 : b.spent - a.spent);
...
const maxMonthlySpend = Math.max(0, ...monthlySpend.map(([, value]) => value)), latestMonthlySpend = monthlySpend.at(-1)?.[1] || 0, previousMonthlySpend = monthlySpend.at(-2)?.[1] || 0, monthlyTrend = previousMonthlySpend ? (latestMonthlySpend - previousMonthlySpend) / previousMonthlySpend * 100 : 0;
...
<section className="spendTimeline compactTrend"><div className="spendTrendHead"><div><h2>Spend over time</h2><p>{monthlySpend.length ? `${monthlySpend.length} ${monthlySpend.length === 1 ? "month" : "months"} within the current filters` : "Monthly spending within the current filters"}</p></div><div className="trendSummary"><span>LATEST MONTH</span><b>{money(latestMonthlySpend)}</b><small className={monthlyTrend > 0 ? "trendUp" : "trendDown"}>{previousMonthlySpend ? `${monthlyTrend > 0 ? "↑" : "↓"} ${Math.abs(monthlyTrend).toFixed(0)}% vs previous month` : "No previous month comparison"}</small></div><button onClick={() => setView("transactions")}>{txs.filter(reviewNeeded).length} need review →</button></div>{monthlySpend.length ? <div className="spendChartViewport"><div className="spendChart" style={{ minWidth: monthlySpend.length > 8 ? `${monthlySpend.length * 70}px` : "100%", gridTemplateColumns: `repeat(${monthlySpend.length}, minmax(52px, 1fr))` }}>{monthlySpend.map(([month, value]) => <div className="spendColumn" key={month} tabIndex={0} role="img" aria-label={`${shortMonth(month)} spending ${money(value)}`}><div className="barTrack"><i style={{ height: `${Math.max(6, maxMonthlySpend ? value / maxMonthlySpend * 100 : 0)}%` }}><span>{money(value)}</span></i></div><small>{shortMonth(month)}</small></div>)}</div></div> : <div className="trendEmpty">Import transactions or change the filters to see your monthly spending trend.</div>}</section>
...
</article>)}{!spend.length && <div className="emptyState categoryTileEmpty">No spending categories match the current filters.</div>}</section>
</> : view === "budget" ? <>
