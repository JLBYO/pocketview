export const tabGuides: Record<string, { name: string; purpose: string; steps: { title: string; text: string }[]; note: string }> = {
    ...
    master: {
        ...
        steps: [
            ...
            { title: "Share An Assistant Snapshot", text: "Choose V2 for all saved transactions plus your financial plan, or V1 for transactions only. V2 includes saved category guides, your explicitly chosen savings target, estimated recurring payments and unconfirmed transfer matches; it never invents wages. Wait for local saving to finish, then download or manually publish. Filters and unsaved drafts are excluded. Publishing does not connect the assistant automatically." },
